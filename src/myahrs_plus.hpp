...
//            std::string& str_message = &ascii_frame;

            StringUtil::split(tokens, ascii_frame, ',');
//            DBG_PRINTF(debug, "%s\n", tokens[0].c_str());
    ...
    class iProtocol : public iAsciiProtocol
    {
    public:
        iProtocol(){}
        virtual ~iProtocol() {}
    ...
    struct ImuData
    {
        Type ax, ay, az, mx, my, mz, gx, gy, gz;

        ImuData() {
            ax = ay = az = mx = my = mz = gx = gy = gz = 0;
        }
...
//            }
    ...
    public:
        ...
        /********** oCamS_BNO055 **********/
        bool cmd_data_format(const char* asc_output=0, int timeout_msec=500) {
			std::string command = std::string("@") + std::string(asc_output) + std::string("\r\n");
			return send_command(command.c_str(), timeout_msec);
        }
        /**********************************/
    ...
    private:
        bool send_command(std::string command_string, int timeout_msec) {
            ...
            if(serial.Write((unsigned char*)command_string.c_str(), command_string.length()) <= 0) {
            ...
            do {
				...
				cmd_tokens.clear();
				StringUtil::split(cmd_tokens, command_string.c_str(), '\r');
            ...
            if(it != ascii_handler_rsp_map.end()) {
                ...
                if(StringUtil::extract_attributes(attributes, tokens) > 0) {
                	DBG_PRINTF(debug, "extract_attributes\n");
                    LockGuard _l(mutex_attribute);
        ...
        // $QUATIMU,53, 0.0424,-0.1791,0.2366,0.9540, -0.3636,0.0027,-0.9260, 0.0156,0.1537,0.2896, 212.2648,-72.7573,168.2144, 36.8 * 7F
        // $LMGQUA,1706673,-97,944,253,-819,-1316,273,0,0,0,5575,-94,-1174
        bool ascii_update_lmgqua(std::vector<std::string>& tokens) {
            static const int DATA_NUM_QUAT = 4;
            static const int DATA_NUM_IMU = 9;
            if(tokens.size() != DATA_NUM_QUAT + DATA_NUM_IMU + 2 ) {
            	DBG_PRINTF(debug, "tokens.size() = %d\n",tokens.size());
        ...
        bool binary_update_sequence(iNodeParser::Node& node) {
            sensor_data.time_stamp = node.list[0].value.ui8;
